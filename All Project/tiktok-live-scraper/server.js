...
app.post('/api/start', async (req, res) => {
    ...
    try {
        ...
        currentConfig = { url, duration, interval, headless, authMode, stealth, chromePath, browser, mode, replyCooldown, replyOnQuestion, replyTemplates, blacklist, whitelist, viplist, webhooks, aiWebhookUrl, hostUsername };
        comments = []; // Reset comments

        // เริ่ม scraper process
        await startScraper(url, duration, interval, headless, authMode, stealth, chromePath, browser || 'chromium', mode || 'read', replyCooldown, replyOnQuestion, replyTemplates, blacklist, whitelist, viplist, webhooks, aiWebhookUrl, hostUsername);
...
app.get('/api/status', (req, res) => {
    res.json({
        ...
        config: currentConfig,
        commentsCount: comments.length,
        uptime: scraperProcess ? process.uptime() : 0
    });
});
...
app.get('/api/comments', (req, res) => {
    ...
    const limitedComments = comments.slice(-limit).reverse(); // ล่าสุดก่อน
...
async function startScraper(url, duration, interval, headless, authMode, stealth, chromePath, browser = 'chromium', mode = 'read', replyCooldown = 5, replyOnQuestion = true, replyTemplates = [], blacklist = [], whitelist = [], viplist = [], webhooks = [], aiWebhookUrl = null, hostUsername = null) {
    ...
    console.log(`🚀 Starting scraper with browser: ${browser} | mode: ${mode}`);
    console.log('🚀 Starting scraper with config:', { 
        ...
        authMode,
        mode,
        stealth: stealth ? '✓ enabled' : '✗ disabled',
        chromePath: chromePath || 'none'
    });
        ...
        try {
            if (output.startsWith('{') || output.startsWith('[')) {
                ...
                if (Array.isArray(parsed)) {
                    parsed.forEach(comment => addComment(comment));
                } else if (parsed.username && parsed.comment) {
                    addComment(parsed);
                }
            }
        } catch (e) {
    ...
    scraperProcess.on('exit', (code, signal) => {
        console.log(`🛑 Scraper process exit (code: ${code}, signal: ${signal})`);
        if (isRunning) {
            isRunning = false;
            scraperProcess = null;
        }
...
function stopScraper() {
    if (scraperProcess) {
        ...
        try {
            // Windows ใช้ taskkill
            if (process.platform === 'win32') {
                // Kill process tree (รวม child processes)
                const { exec } = require('child_process');
                ...
                // Linux/Mac ใช้ SIGTERM แล้วตาม SIGKILL
...
async function saveComments() {
    ...
    try {
        ...
        const filename = `comments_${date}_${Date.now()}.json`;
