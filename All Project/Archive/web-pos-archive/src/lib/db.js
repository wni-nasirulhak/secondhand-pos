...
export async function getDB() {
  ...
  // Local Fallback for next dev (only in Node.js runtime)
  if (!localDb) {
    const isNode = typeof process !== 'undefined' && process.versions?.node;
    if (!isNode) {
      console.warn('Developing in non-Node runtime (Edge?) but D1 binding (DB) is missing.');
      return null;
    }
...
export async function query(sql, params = []) {
  ...
  if (!db) {
    console.error('Database (D1 or SQLite) is not available.');
    return { results: [] };
  }
